        ...
        className='bg-white rounded-2xl p-4 shadow-sm border flex gap-4'>
            ...
            <div className='flex flex-col justify-between'>
                ...
                <div className='flex items-center mt-5 gap-4 border rounded-full px-3 py-2'>
                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => decreaseQty(item.cartItemId)}>
                        <Minus size={18} />
                    </motion.button>

                    <span>{item.quantity}</span>

                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => increaseQty(item.cartItemId)}>
                        <Plus size={18} />
                    </motion.button>
                </div>
